...
const AIGuideDemo: React.FC<{ onComplete?: () => void }> = ({ onComplete }) => {
  ...
  // Fallback default timeline if not found
  const timeline = DEMO_PLAYLIST.find(s => s.id === 'guide')?.timeline || { workbenchStay: 2000, intro: 2000, userSpeech: 12000, thinking: 25000, feedback: 30000, report: 42000, complete: 52000 };

  useEffect(() => {
    let autoStartTimer: ReturnType<typeof setTimeout>;
    // FIXED: Use timeline.workbenchStay for the initial delay
    if (mode === 'intro' && !simulationRef.current) { 
        autoStartTimer = setTimeout(() => startSimulation(), timeline.workbenchStay || 2000); 
    }
                ...
                <div className="flex-1 bg-slate-800/50 rounded-3xl border border-white/5 overflow-hidden relative">
                    <div ref={chatContainerRef} className="absolute inset-0 p-6 lg:p-8 overflow-y-auto space-y-6 custom-scrollbar scroll-smooth">
                       {transcript.map((t, i) => (
                          ...
                          >
                             <div className={`max-w-[85%] p-5 rounded-3xl text-lg lg:text-xl leading-relaxed ${
                                t.sender === 'ai' 
                                ? 'bg-slate-700/50 text-slate-200 rounded-tl-none' 
                                : 'bg-blue-600 text-white rounded-tr-none'
                             }`}>
                                <div className="flex items-center space-x-2 mb-2 opacity-50 text-sm uppercase tracking-wider font-bold">
                                   {t.sender === 'ai' ? <Radio size={16}/> : <Mic size={16}/>}
                       ...
                       {avatarState === 'listening' && (
                          <div className="flex justify-end">
                             <div className="bg-blue-600/20 text-blue-400 p-4 rounded-3xl rounded-tr-none flex items-center space-x-2">
