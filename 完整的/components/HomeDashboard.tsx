...
const ChinaMapAlertSection = () => {
  ...
  // FIX 2: Enhanced Smart Positioning (Vertical + Horizontal)
  const isLowerHalf = alert ? alert.y > 50 : false;
  const isRightSide = alert ? alert.x > 60 : false;
  const isLeftSide = alert ? alert.x < 40 : false;

  // Horizontal Shift Logic
  // If dot is on right (>60%), shift box left (-85%). 
  // If dot is on left (<40%), shift box right (-15%).
  // Default center (-50%).
  const xOffset = isRightSide ? '-85%' : (isLeftSide ? '-15%' : '-50%');
  
  // Arrow Compensation Logic
  // If box moved left, arrow needs to move right (to 85%).
  // If box moved right, arrow needs to move left (to 15%).
  const arrowLeft = isRightSide ? '85%' : (isLeftSide ? '15%' : '50%');
          ...
          <div className="relative w-full h-full select-none flex items-center justify-center">
            ...
            <AnimatePresence mode="wait">
              {alert && (
                <motion.div 
                    key={alert.id} 
                    // FIX: Full Smart Positioning (X & Y)
                    initial={{ opacity: 0, y: 10, x: '-50%', scale: 0.8 }} 
                    animate={{ 
                        opacity: 1, 
                        y: isLowerHalf ? -135 : 15, // Up (-135) or Down (15)
                        x: xOffset, // Left/Right/Center Shift
                        scale: 1 
                    }} 
                ...
                >
                   <div className={`w-64 ${themeStyles.bg} bg-opacity-95 backdrop-blur-xl border ${themeStyles.border} rounded-xl p-4 shadow-2xl relative overflow-hidden ${themeStyles.shadow}`}>
                      ...
                      {/* FIX: Smart Arrow Positioning */}
                      <div 
                        className={`absolute transform -translate-x-1/2 rotate-45 w-4 h-4 ${themeStyles.arrow} ${themeStyles.border} ${isLowerHalf ? '-bottom-2 border-r border-b' : '-top-2 border-l border-t'}`}
            ...
            >
              ...
              >
                 ...
                 <div className="flex-none p-8 border-b border-white/5 flex items-start gap-5 bg-gradient-to-r from-purple-900/20 to-indigo-900/10">
                    ...
                    <div><h2 className="text-2xl font-bold text-white mb-1 flex items-center gap-2">AI 智能运营报告 <span className="text-xs font-normal text-purple-300 bg-purple-500/10 px-2 py-0.5 rounded-full border border-purple-500/20">Retail AI Analysis</span></h2><p className="text-slate-400 text-sm">基于大盘数据生成的深度诊断与策略建议</p></div>
                 </div>
                 ...
                 <div className="flex-none p-6 border-t border-white/5 bg-slate-900/50 flex justify-between items-center">
                    <div className="flex items-center gap-2"><div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></div><span className="text-xs text-slate-400">Generated by Retail AI</span></div>
