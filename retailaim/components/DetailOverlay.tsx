...
const DetailOverlay: React.FC<DetailOverlayProps> = ({ moduleData, onClose, onComplete }) => {
  ...
  const renderContent = () => {
    switch (moduleData.id) {
      ...
      case ModuleType.DATA_DASHBOARD:
        // Wrap HomeDashboard to fit in overlay and pass onComplete
        return (
  ...
  return (
    <motion.div
      ...
      className="fixed inset-0 z-40 bg-black/60 flex items-center justify-center p-6 md:p-12"
    >
      <motion.div 
        initial={{ scale: 0.9, y: 20 }}
        animate={{ scale: 1, y: 0 }}
        exit={{ scale: 0.9, y: 20 }}
        className="w-full max-w-7xl h-[85vh] bg-slate-900/90 border border-white/10 rounded-3xl shadow-2xl overflow-hidden flex flex-col relative"
        style={{ boxShadow: `0 0 100px -20px ${moduleData.color.includes('blue') ? '#3b82f650' : moduleData.color.includes('pink') ? '#ec489950' : '#a855f750'}`}}
      >
        {/* Close Button */}
        <button 
          onClick={onClose}
        ...
        {/* Content Container */}
        <div className="flex-1 p-0 overflow-hidden h-full">
          {renderContent()}
        </div>
      </motion.div>
    </motion.div>
