...
const BUSINESS_MODULES = [
  { title: "O2O 平台运营", en: "PLATFORM OPERATIONS", icon: BarChart3, border: "border-blue-500", glow: "group-hover:shadow-blue-500/50" },
  { title: "全域供给管理", en: "TOTAL SUPPLY MANAGEMENT", icon: MapIcon, border: "border-emerald-500", glow: "group-hover:shadow-emerald-500/50" },
...
const CompanyIntro: React.FC<{ active?: boolean; onComplete?: () => void }> = ({ active = false }) => {
  return (
    <div className={`h-full w-full relative overflow-hidden flex flex-col font-sans text-white transition-all duration-1000 ${active ? 'blur-2xl scale-110 opacity-0 pointer-events-none' : 'scale-100 opacity-100'}`}>
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center w-full h-full max-w-[2400px] mx-auto p-12 lg:p-16 pb-32">
        ...
        >
          ...
          <div className="flex items-center justify-center gap-8 mb-4">
             {/* Logo Icon - Ruitai Brand Logo (No Distortion) */}
          ...
          <div className="flex flex-col items-center gap-2">
            <h2 className="text-5xl lg:text-6xl font-bold text-white tracking-wide drop-shadow-[0_0_20px_rgba(255,255,255,0.3)]">
               全域零售 AI 解决方案专家
            </h2>
            <h3 className="text-2xl lg:text-3xl font-light text-slate-300 tracking-[0.2em] uppercase opacity-90">
               打通即时零售与线下门店协同增长的全渠道引擎
            </h3>
          </div>
        </motion.div>

        {/* 2. CORE METRICS (5 Columns - High Impact Visibility) */}
        <div className="w-full grid grid-cols-5 gap-8 mb-16 relative px-4 items-end">
              {METRICS_LAYOUT.map((m, i) => {
                 ...
                 // Dynamic classes for size and emphasis
                 let heightClass = "h-[220px]";
                 let valueSizeClass = "text-7xl lg:text-8xl"; // Huge numbers
                 let labelSizeClass = "text-2xl lg:text-3xl font-bold text-slate-200"; // Readable labels
                 
                 // Animation Class
                 let animationClass = "animate-breathe-border bg-white/5";

                 if (isSuperHero) {
                    heightClass = "h-[320px]";
                    valueSizeClass = "text-8xl lg:text-9xl";
                    labelSizeClass = "text-3xl lg:text-4xl font-extrabold text-amber-100";
                    animationClass = ""; // Handled manually for gold border
                 } else if (isHero) {
                    heightClass = "h-[260px]";
                    valueSizeClass = "text-7xl lg:text-8xl";
                    labelSizeClass = "text-2xl lg:text-3xl font-bold text-cyan-50";
                 }
                 ...
                 return (
                    <motion.div
                      ...
                      transition={{ delay: 0.5 + i * 0.1, type: "spring", stiffness: 60 }}
                      className={`relative w-full ${isSuperHero ? 'z-20' : 'z-10'}`}
                    >
                       {isSuperHero ? (
                          // 300亿+ Card: Golden Flowing Border
                          <div className={`relative ${heightClass} w-full rounded-3xl overflow-hidden shadow-[0_0_60px_rgba(251,191,36,0.3)]`}>
                              ...
                              <div className="absolute inset-[3px] bg-slate-900/90 backdrop-blur-xl rounded-[22px] flex flex-col items-center justify-center p-4">
                                  ...
                                  <div className="flex flex-col items-center mt-2 relative z-10">
                                      <div className={`tracking-widest leading-tight ${labelSizeClass}`}>
                                          {m.label}
                                      </div>
                                      <div className="text-sm lg:text-base font-mono text-amber-200/60 uppercase tracking-widest mt-1">
                                          {m.subLabel}
                                      </div>
                                  </div>
                              </div>
                          ...
                          // Standard & Hero Cards: Breathing Border
                          <div className={`
                             relative flex flex-col items-center justify-center text-center
                             backdrop-blur-xl border-2 rounded-3xl transition-all duration-500
                             w-full shadow-2xl ${heightClass} ${m.glow || ''}
                             ${animationClass} border-white/10
                          `}>
                              {/* Inner Glow Effect */}
                              ...
                              <div className="flex flex-col items-center mt-2 relative z-10">
                                  ...
                                  <div className="text-sm lg:text-base font-mono text-slate-400 uppercase tracking-widest mt-1 opacity-80">
                                      {m.subLabel}
                                  </div>
                              </div>
                          </div>
           ...
           <div className="grid grid-cols-3 gap-10 w-full">
              {BUSINESS_MODULES.map((mod, i) => (
                 <motion.div
                   ...
                   className={`
                      relative group cursor-pointer overflow-hidden animate-float
                      h-48 bg-slate-900/40 backdrop-blur-2xl border border-white/20 rounded-3xl
                      flex flex-col items-center justify-center text-center p-8
                      hover:border-${mod.border.split('-')[1]}-400/50 transition-all duration-300 shadow-2xl
                      ${mod.glow}
                   `}
                   style={{ animationDelay: `${i * 1.5}s` }} // Staggered floating
                 >
                    {/* Hover Gradient */}
                    <div className={`
                       absolute inset-0 bg-gradient-to-br from-transparent via-transparent to-${mod.border.split('-')[1]}-500/20 
        ...
        {/* 4. BRAND WALL (Floating Pill) */}
        <motion.div 
           initial={{ opacity: 0, y: 30 }} 
           animate={{ opacity: 1, y: 0 }} 
           transition={{ delay: 2, duration: 1 }}
           className="w-full max-w-[1600px] mt-4"
        >
                  ...
                  <div className="flex-1 overflow-hidden relative">
                    <div className="flex animate-[marquee_40s_linear_infinite] items-center gap-10 h-24">
                      {[...BRAND_LOGOS, ...BRAND_LOGOS].map((brand, i) => (
                          <div 
                            key={i} 
                            className="flex flex-col items-center gap-2 shrink-0 opacity-70 hover:opacity-100 transition-all cursor-default group"
                          >
                             {/* Logo Placeholder */}
                      ...
                      <div className="flex items-center gap-4 shrink-0 opacity-100 pl-6 border-l border-white/10 ml-4">
                           <div className="w-12 h-12 bg-cyan-500/20 border border-cyan-500/30 rounded-full flex items-center justify-center animate-pulse shadow-[0_0_20px_rgba(34,211,238,0.2)]">
                               <Plus size={24} className="text-cyan-400" />
                           </div>
                           <div className="flex flex-col">
                               <span className="text-2xl font-bold text-cyan-300 italic tracking-wider">And More...</span>
                               <span className="text-xs text-cyan-400/70 font-mono tracking-wider">GLOBAL NETWORK</span>
                           </div>
                      </div>
