...
// --- MOCK DATA ---
// ... (Data definitions skipped for brevity, keeping existing data)
...
const getAlertTheme = (theme: string) => {
  switch (theme) {
...
// --- SUB-COMPONENTS ---
// (ChinaMapAlertSection and ContributionRank components remain unchanged)
const ChinaMapAlertSection = () => {
  const [currentAlert, setCurrentAlert] = useState<number | null>(0);
    ...
    <div className="relative w-full h-full bg-slate-900/40 backdrop-blur rounded-2xl border border-white/10 overflow-hidden flex flex-col">
      ...
      <div className="p-3 border-b border-white/5 flex justify-between items-center bg-slate-800/30 z-10">
        <h3 className="font-bold text-white text-sm flex items-center gap-2">
          <AlertTriangle size={14} className="text-red-400"/> 全域供给预警
        </h3>
      </div>
      ...
      <div className="flex-1 relative p-2 flex items-center justify-center overflow-hidden">
          ...
          <div className="relative w-full h-full select-none flex items-center justify-center">
                ...
                return (
                  <div key={index} className="absolute transform -translate-x-1/2 -translate-y-1/2" style={{ left: `${item.x}%`, top: `${item.y}%` }}>
                    <div className={`relative flex items-center justify-center group`}>
                        ...
                        {/* Ripple for stock alerts or active item */}
                        {(item.type === 'STOCK' || currentAlert === index) && (
                            <div className={`absolute w-12 h-12 rounded-full border ${itemStyles.border} animate-ping opacity-50`}></div>
                        )}
                    </div>
                  </div>
                );
            ...
            {/* Dynamic Tooltip - HIGHLIGHTED & COLORED */}
                ...
                >
                   <div className={`w-64 ${themeStyles.bg} bg-opacity-95 backdrop-blur-xl border ${themeStyles.border} rounded-xl p-4 shadow-2xl relative overflow-hidden ${themeStyles.shadow}`}>
                      ...
                      <div className="flex justify-between items-start mb-2">
                        <span className={`text-xs font-bold flex items-center gap-1 opacity-90 ${themeStyles.text === 'text-yellow-950' ? 'text-yellow-900' : 'text-white'}`}>
                           <MapPin size={12}/> {alert.region}
                        </span>
                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold shadow-sm bg-white/20 backdrop-blur-md ${themeStyles.text}`}>
                           {alert.platform}
                        </span>
                      </div>

                      {/* Message - Highlighted */}
                      <div className={`text-lg font-extrabold mb-3 leading-tight ${themeStyles.text}`}>
                          {alert.message}
                      </div>
                      ...
                      <div className={`rounded-lg p-3 flex items-start gap-2 ${themeStyles.actionBg}`}>
                         ...
                         <div className={`text-xs font-bold leading-snug ${themeStyles.text} opacity-90`}>
                            {alert.action}
                         </div>
                      </div>
...
const ContributionRank = () => {
  const [activeTab, setActiveTab] = useState<'city' | 'merchant'>('city');
  ...
  return (
    <div className="bg-slate-900/60 backdrop-blur border border-white/10 rounded-2xl p-4 h-full flex flex-col">
       ...
       <div className="flex items-center justify-between mb-4 pb-2 border-b border-white/5">
          <h3 className="font-bold text-white text-sm flex items-center gap-2">
             <Trophy size={14} className="text-yellow-400"/> 贡献排行榜
          </h3>
          <div className="flex bg-slate-800 rounded-lg p-0.5">
             <button 
               onClick={() => setActiveTab('city')}
               className={`px-3 py-1 text-[10px] font-bold rounded-md transition-all ${activeTab === 'city' ? 'bg-slate-600 text-white shadow' : 'text-slate-400 hover:text-white'}`}
             >
               城市
             </button>
             <button 
               onClick={() => setActiveTab('merchant')}
               className={`px-3 py-1 text-[10px] font-bold rounded-md transition-all ${activeTab === 'merchant' ? 'bg-slate-600 text-white shadow' : 'text-slate-400 hover:text-white'}`}
             >
               商家
             </button>
          </div>
             ...
             <div key={idx} className="flex items-center justify-between p-3 rounded hover:bg-white/5 transition-colors group border-b border-white/5 last:border-0">
                 <div className="flex items-center gap-3">
                    <div className={`w-6 h-6 rounded flex items-center justify-center text-xs font-bold ${
                       ...
                       idx === 2 ? 'bg-orange-700 text-orange-200' : 'bg-slate-800 text-slate-500'
                    }`}>
                       {idx + 1}
                    </div>
                 ...
                 <div className="text-right">
                    ...
                    <div className={`text-[10px] font-bold ${item.trend.startsWith('+') ? 'text-emerald-500' : 'text-rose-500'}`}>
                       {item.trend}
                    </div>
                 </div>
             </div>
...
const HomeDashboard: React.FC<HomeDashboardProps> = ({ activeModule, onComplete }) => {
  ...
  useEffect(() => {
    if (!activeModule) { // Only if dashboard is visible
  ...
  const handleSimulateGeneration = () => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
         ...
         setTimeout(() => {
             setAiReportOpen(false); // Close AI Modal first
             
             // Then Close the entire Dashboard Module
             setTimeout(() => {
  ...
  return (
    <div className="relative w-full h-full bg-slate-900 overflow-hidden text-sans">
      ...
      >
        ...
        {/* Row 1: Branding, AI Button & Platform Status */}
        <div className="flex items-center justify-between mb-4">
           ...
           <div className="flex items-center space-x-4">
              ...
              <div className="flex flex-col justify-center">
                  ...
                  <p className="text-slate-200 text-xs font-light tracking-wide text-shadow-sm">
                       O2O 全域生意指挥舱
                  </p>
              </div>
           </div>

           {/* Zone A: AI Button & Platform Status (Badges) */}
           <div className="flex items-center gap-4">
              {/* AI Analysis Button with Bubble - LEFT ALIGNED BUBBLE */}
              <div className="relative">
                 {/* Tooltip positioned strictly to the left of the button */}
                 ...
                 <button 
                     ...
                     className="flex items-center gap-2 px-4 py-1.5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white rounded-full shadow-lg shadow-purple-900/30 transition-all text-xs font-bold border border-white/10 group"
                 >
                     <Sparkles size={14} className="text-purple-100 group-hover:scale-110 transition-transform" />
                     AI 智能解读
                 </button>
              </div>
              ...
              <div className="flex items-center gap-3">
                <div className="bg-yellow-500/20 text-yellow-400 border border-yellow-500/50 px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1.5 shadow-lg shadow-yellow-500/10">
                    <div className="w-2 h-2 bg-yellow-500 rounded-full animate-pulse"></div> 美团
                </div>
                <div className="bg-orange-500/20 text-orange-400 border border-orange-500/50 px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1.5 shadow-lg shadow-orange-500/10">
                    <div className="w-2 h-2 bg-orange-500 rounded-full animate-pulse"></div> 淘宝
                </div>
                <div className="bg-red-500/20 text-red-400 border border-red-500/50 px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1.5 shadow-lg shadow-red-500/10">
                    <div className="w-2 h-2 bg-red-500 rounded-full animate-pulse"></div> 京东
                </div>
                <div className="bg-blue-500/20 text-blue-400 border border-blue-500/50 px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1.5 shadow-lg shadow-blue-500/10">
                    <div className="w-2 h-2 bg-blue-500 rounded-full animate-pulse"></div> 饿了么
                </div>
              </div>
           </div>
        </div>
        ...
        <div className="grid grid-cols-12 gap-4 mb-6 bg-slate-900/60 backdrop-blur-md p-4 rounded-2xl border border-white/10 shadow-2xl relative overflow-hidden">
             ...
             {/* Group 1: Business (生意) - 3 Cols */}
             <div className="col-span-3 border-r border-white/10 px-4 flex flex-col justify-center relative">
                <div className="flex items-center gap-2 mb-1">
                   <Database size={14} className="text-blue-400"/>
                   <span className="text-xs text-slate-400 font-bold uppercase tracking-wider">全域 GMV</span>
                </div>
                <div className="flex items-baseline gap-2">
                   <span className="text-3xl font-bold text-white font-mono tracking-tight">¥{gmv.toLocaleString()}</span>
                   <span className="text-xs font-bold text-emerald-400">+12.4%</span>
                </div>
                <div className="flex items-center gap-2 mt-1 opacity-70">
                   <span className="text-[10px] text-slate-400">活动 GMV:</span>
                   <span className="text-xs font-mono text-white">¥3.52M (32%)</span>
                </div>
             </div>

             {/* Group 2: Supply (供给) - 3 Cols */}
             <div className="col-span-3 border-r border-white/10 px-4 flex flex-col justify-center">
                <div className="flex items-center gap-2 mb-1">
                   <Package size={14} className="text-cyan-400"/>
                   <span className="text-xs text-slate-400 font-bold uppercase tracking-wider">全网供给</span>
                </div>
                <div className="grid grid-cols-2 gap-2">
                   <div>
                      <div className="text-2xl font-bold text-white font-mono">128.4k</div>
                      <div className="text-[10px] text-slate-500 uppercase font-bold">全网销量</div>
                   </div>
                   <div>
                      <div className="text-2xl font-bold text-cyan-400 font-mono">2,410</div>
                      <div className="text-[10px] text-slate-500 uppercase font-bold">动销门店</div>
                   </div>
                </div>
             </div>

             {/* Group 3: Activity Efficiency (活动能效) + Alert - 6 Cols */}
             <div className="col-span-6 px-4 flex flex-col justify-center">
                <div className="flex items-center gap-2 mb-2">
                   <Activity size={14} className="text-purple-400"/>
                   <span className="text-xs text-slate-400 font-bold uppercase tracking-wider">活动能效</span>
                </div>
                ...
                <div className="flex items-center justify-between">
                   ...
                   <div className="flex gap-8">
                      <div>
                         <div className="text-2xl font-bold text-white font-mono">¥249,100</div>
                         <div className="text-[10px] text-slate-500 uppercase font-bold">活动费用</div>
                      </div>
                      <div>
                         <div className="text-2xl font-bold text-rose-400 font-mono">15.6%</div>
                         <div className="text-[10px] text-slate-500 uppercase font-bold">费比</div>
                      </div>
                      <div>
                         <div className="text-2xl font-bold text-white font-mono">6.4</div>
                         <div className="text-[10px] text-slate-500 uppercase font-bold">ROI</div>
                      </div>
                   </div>

                   {/* Alert Box - Integrated here */}
                   <div className="bg-red-950/40 border border-red-500/30 rounded-xl px-3 py-2 flex items-center gap-3 cursor-pointer hover:bg-red-950/60 transition-colors">
                       ...
                       <div>
                          <div className="text-[10px] font-bold text-red-400 uppercase tracking-wider flex items-center gap-1">
                             <AlertOctagon size={10} /> 异常预警 (Critical)
                          </div>
        ...
        <div className="flex-1 grid grid-cols-12 gap-6 pb-20 min-h-0">
          ...
          {/* LEFT COLUMN: Split (Efficiency + Products) */}
          <div className="col-span-3 flex flex-col space-y-4 min-h-0">
              
              {/* Module A: Platform Distribution (Top) */}
              <div className="flex-1 bg-slate-900/60 backdrop-blur border border-white/10 rounded-2xl p-4 flex flex-col overflow-hidden">
                  <div className="flex justify-between items-center mb-3 shrink-0">
                      <h3 className="font-bold text-white flex items-center gap-2 text-sm"><LayoutTemplate size={16} className="text-cyan-400"/> 平台分布 (GMV)</h3>
                  </div>
                  <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 space-y-2">
                      {PLATFORM_MATRIX_DATA.map((p, i) => (
                          <div key={i} className="flex items-center justify-between p-2 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 transition-colors group">
                              ...
                              {/* Right: ROI Badge */}
                              <div className={`px-2 py-0.5 rounded text-[10px] font-bold font-mono ${p.roi > 5 ? 'bg-emerald-500/20 text-emerald-400' : 'bg-orange-500/20 text-orange-400'}`}>
                                  {p.roi}
                              </div>
                          </div>
                      ))}
              ...
              {/* Module B: Target Products (Bottom) - Moved from Right */}
              <div className="flex-1 bg-slate-900/60 backdrop-blur border border-white/10 rounded-2xl p-4 flex flex-col overflow-hidden">
                 <div className="flex justify-between items-center mb-3 shrink-0">
                    <h3 className="font-bold text-white text-sm flex items-center gap-2">
                       <Rocket size={16} className="text-purple-400"/> 目标商品监控
                    </h3>
                 </div>
                 <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 space-y-2">
                       ...
                       <div key={i} className="group bg-white/5 p-2 rounded-lg hover:bg-white/10 transition-colors border border-white/5">
                          <div className="flex justify-between items-center mb-1">
                             <div className="flex items-center gap-2">
                                <div className={`w-4 h-4 rounded flex items-center justify-center text-[9px] font-bold ${i<3 ? 'bg-yellow-500/20 text-yellow-400' : 'bg-slate-700 text-slate-400'}`}>{i+1}</div>
                                <span className="text-xs font-bold text-slate-200">{sku.name}</span>
                             </div>
                             <div className="text-xs font-mono text-white font-bold">{sku.gmv}</div>
                          </div>
                          <div className="flex justify-between items-center">
                             <div className="w-24 h-1 bg-slate-700 rounded-full overflow-hidden">
                                <div className="h-full bg-purple-500" style={{ width: `${80 - i*10}%`}}></div>
                             </div>
                             <span className={`text-[9px] font-bold ${sku.trend.startsWith('+') ? 'text-emerald-400' : 'text-rose-400'}`}>{sku.trend}</span>
                          </div>
                       </div>
                    ))}
                 </div>
             ...
             <div className="bg-slate-900/80 backdrop-blur rounded-2xl p-4 border border-white/10 flex flex-col h-[35%]">
                <div className="flex justify-between items-center mb-2">
                   <div className="flex items-center gap-2 border-l-4 border-cyan-400 pl-3">
                      <h3 className="font-bold text-white text-sm">趋势图 (Trend)</h3>
                   </div>
                   <div className="flex items-center gap-4 text-[10px]">
                      <div className="flex items-center gap-1.5">
                         <div className="w-2 h-2 bg-cyan-500 rounded-sm"></div>
                         <span className="text-slate-300">GMV</span>
                      </div>
                      <div className="flex items-center gap-1.5">
                         <div className="w-2 h-0.5 bg-purple-400 rounded-full"></div>
                ...
                <div className="flex-1 w-full min-h-0">
                   <ResponsiveContainer width="100%" height="100%">
                      <ComposedChart data={SEVEN_DAY_DATA} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                         ...
                         />
                         <Bar yAxisId="left" dataKey="gmv" fill="#22d3ee" barSize={16} radius={[2, 2, 0, 0]} />
                         <Line yAxisId="right" type="monotone" dataKey="roi" stroke="#a855f7" strokeWidth={2} dot={{r: 2}} />
                      </ComposedChart>
                   </ResponsiveContainer>
                </div>
             </div>

             {/* Map Section - Pure Stock Alerts */}
          ...
          {/* RIGHT COLUMN: Full Height Rankings */}
      ...
      <AnimatePresence>
          {aiReportOpen && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="absolute inset-0 z-50 bg-slate-900/90 backdrop-blur-xl flex items-center justify-center p-8"
            >
              <motion.div 
                initial={{ scale: 0.9, y: 20 }}
                animate={{ scale: 1, y: 0 }}
                exit={{ scale: 0.9, y: 20 }}
                // Modified: Smaller width (max-w-4xl) and height (h-[75vh])
              ...
              >
                 {/* Close Button */}
                 <button onClick={() => setAiReportOpen(false)} className="absolute top-6 right-6 text-slate-400 hover:text-white z-50">
                    <X size={24}/>
                 </button>
                 ...
                 <div className="flex-none p-8 border-b border-white/5 flex items-start gap-5 bg-gradient-to-r from-purple-900/20 to-indigo-900/10">
                    <div className="w-14 h-14 bg-gradient-to-br from-purple-500 to-indigo-600 rounded-2xl flex items-center justify-center shadow-lg shadow-purple-900/50">
                       <Bot size={32} className="text-white" />
                    </div>
                    <div>
                       <h2 className="text-2xl font-bold text-white mb-1 flex items-center gap-2">
                          AI 智能运营报告
                          <span className="text-xs font-normal text-purple-300 bg-purple-500/10 px-2 py-0.5 rounded-full border border-purple-500/20">Gemini Analysis</span>
                       </h2>
                       <p className="text-slate-400 text-sm">基于大盘数据生成的深度诊断与策略建议</p>
                    </div>
                 </div>
                 ...
                 <div ref={reportContainerRef} className="flex-1 overflow-y-auto custom-scrollbar p-8 scroll-smooth">
                    {isGenerating ? (
                       <div className="h-full flex flex-col items-center justify-center">
                          ...
                          <h3 className="text-xl font-bold text-white mb-2">正在生成深度分析报告...</h3>
                          ...
                          <p className="text-slate-500 text-sm mt-4 font-mono">Analyzing {generationProgress}%</p>
                       </div>
                    ) : (
                       <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                          ...
                          <div className="bg-slate-800/30 rounded-2xl p-6 border border-white/5">
                             <div className="flex items-center gap-2 mb-4">
                                ...
                                <h3 className="text-lg font-bold text-white">核心洞察 (Executive Summary)</h3>
                             </div>
                             <div className="flex items-start gap-6">
                                <div className="flex-1">
                                   <div className="text-4xl font-mono font-bold text-white mb-1">{AI_ANALYSIS_DATA.summary.gmv}</div>
                                   <div className="text-sm font-bold text-emerald-400 mb-4">{AI_ANALYSIS_DATA.summary.trend}</div>
                                   <p className="text-slate-300 leading-relaxed text-sm">
                                      {AI_ANALYSIS_DATA.summary.text}
                                   </p>
                                </div>
                                <div className="flex flex-col gap-2">
                                   {AI_ANALYSIS_DATA.summary.tags.map((tag, i) => (
                                      <span key={i} className={`px-3 py-1 rounded text-xs font-bold bg-${tag.color}-500/10 text-${tag.color}-400 border border-${tag.color}-500/20 text-center`}>
                                         {tag.label}
                                      </span>
                                   ))}
                                </div>
                          ...
                          <div className="grid grid-cols-2 gap-6">
                             ...
                             <div className="bg-emerald-900/10 border border-emerald-500/20 rounded-2xl p-6">
                                <div className="flex items-center gap-2 mb-4 border-b border-emerald-500/20 pb-2">
                                   ...
                                   <h3 className="text-lg font-bold text-emerald-100">增长引擎 (Growth Drivers)</h3>
                                </div>
                                <div className="space-y-4">
                                   {AI_ANALYSIS_DATA.growth.map((item, i) => (
                                      <div key={i} className="bg-emerald-950/30 p-3 rounded-lg border border-emerald-500/10">
                                         <div className="flex justify-between items-center mb-1">
                                            <span className="text-emerald-200 font-bold text-sm">{item.title}</span>
                                            <span className="text-emerald-400 font-mono font-bold text-xs">{item.value}</span>
                                         </div>
                                         <div className="text-xs text-emerald-100/70 mb-1">{item.sub}</div>
                                         <div className="text-[10px] text-emerald-100/50">{item.desc}</div>
                                      </div>
                                   ))}
                                </div>
                             </div>
                             ...
                             <div className="bg-rose-900/10 border border-rose-500/20 rounded-2xl p-6">
                                <div className="flex items-center gap-2 mb-4 border-b border-rose-500/20 pb-2">
                                   ...
                                   <h3 className="text-lg font-bold text-rose-100">风险阻力 (Risk Factors)</h3>
                                </div>
                                <div className="space-y-4">
                                   {AI_ANALYSIS_DATA.drag.map((item, i) => (
                                      <div key={i} className="bg-rose-950/30 p-3 rounded-lg border border-rose-500/10">
                                         <div className="flex justify-between items-center mb-1">
                                            <span className="text-rose-200 font-bold text-sm">{item.title}</span>
                                            <span className="text-rose-400 font-mono font-bold text-xs">{item.value}</span>
                                         </div>
                                         <div className="text-xs text-rose-100/70 mb-1">{item.sub}</div>
                                         <div className="text-[10px] text-rose-100/50">{item.desc}</div>
                                      </div>
                                   ))}
                                </div>
                             </div>
                          ...
                          <div className="bg-slate-800/30 rounded-2xl p-6 border border-white/5">
                             <div className="flex items-center gap-2 mb-4">
                                <AlertOctagon className="text-yellow-400" size={20}/>
                                <h3 className="text-lg font-bold text-white">问题症结 (Core Issues)</h3>
                             </div>
                             <div className="grid grid-cols-3 gap-4">
                                {AI_ANALYSIS_DATA.issues.map((issue, i) => (
                                   <div key={i} className="bg-white/5 p-4 rounded-xl hover:bg-white/10 transition-colors">
                                      <div className="text-sm font-bold text-yellow-100 mb-2">{i+1}. {issue.title}</div>
                          ...
                          <div className="bg-gradient-to-br from-blue-900/20 to-purple-900/20 rounded-2xl p-6 border border-white/10">
                             <div className="flex items-center gap-2 mb-6">
                                ...
                                <h3 className="text-lg font-bold text-white">下一步计划 (Action Plan)</h3>
                             </div>
                             <div className="space-y-4 relative">
                                {/* Vertical Line */}
                                <div className="absolute left-6 top-2 bottom-2 w-px bg-slate-700"></div>

                                {AI_ANALYSIS_DATA.plan.map((step, i) => (
                                   <div key={i} className="flex items-start gap-4 relative z-10">
                                      <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-white shrink-0 border-4 border-[#0f172a] ${
                                         step.type === 'urgent' ? 'bg-red-500' : step.type === 'optimize' ? 'bg-blue-500' : 'bg-purple-500'
                                      }`}>
                                         {i+1}
                                      </div>
                                      <div className="bg-slate-800/80 p-4 rounded-xl border border-white/5 flex-1 shadow-lg">
                                         <div className="flex items-center gap-2 mb-1">
                                            <span className={`text-xs font-bold px-2 py-0.5 rounded text-white ${
                                               step.type === 'urgent' ? 'bg-red-500/20 text-red-300' : step.type === 'optimize' ? 'bg-blue-500/20 text-blue-300' : 'bg-purple-500/20 text-purple-300'
                                            }`}>
                                               {step.label}
                                            </span>
                                         </div>
                 ...
                 <div className="flex-none p-6 border-t border-white/5 bg-slate-900/50 flex justify-between items-center">
                    <div className="flex items-center gap-2">
                       ...
                       <span className="text-xs text-slate-400">Generated by Google Gemini</span>
                    </div>
                    <div className="flex gap-3">
                       <button onClick={handleExportPDF} className="px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded-lg text-sm font-bold flex items-center gap-2 transition-colors">
                          <Download size={16}/> 导出 PDF
                       </button>
                       <button onClick={() => setAiReportOpen(false)} className="px-6 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-lg text-sm font-bold transition-colors shadow-lg shadow-purple-900/30">
                          确认
                       </button>
                    </div>
                 </div>
