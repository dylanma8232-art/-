...
const MiniProgramRouteView: React.FC<{ onBack: () => void }> = ({ onBack }) => {
    ...
    return (
        <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: "spring", damping: 25 }} className="absolute inset-0 flex flex-col bg-white h-full z-50">
            <div className="bg-white px-6 pt-16 pb-4 flex items-center justify-between border-b border-gray-100 relative z-20 shrink-0">
                ...
                <AnimatePresence>
                    {aiPlanVisible && (
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex-1 flex flex-col space-y-4">
                            ...
                            {/* Map Container - Increased Height and fixed aspect ratio handling */}
                            <div className="h-48 w-full bg-slate-100 rounded-3xl border border-slate-200 relative overflow-hidden shadow-inner shrink-0">
                            ...
                            <div className="space-y-4">
                                <div className="bg-white rounded-xl p-4 border-l-4 border-orange-500 shadow-sm flex items-start space-x-4">
                                    <div className="bg-orange-50 text-orange-500 p-3 rounded-lg font-bold text-sm text-center min-w-[60px] flex flex-col justify-center"><div>STOP</div><div className="text-lg">1</div></div>
                                    <div className="flex-1 py-1">
                                        <div className="flex justify-between items-center mb-1"><span className="font-bold text-lg text-slate-800">罗森 (丹棱街)</span><span className="text-[10px] text-orange-500 bg-orange-50 px-2 py-1 rounded-full font-bold whitespace-nowrap">库存预警</span></div>
                                        <div className="text-sm text-slate-500">任务: 盘点冷柜饮料库存</div>
                                    </div>
                                </div>
                                <div className="bg-white rounded-xl p-4 border-l-4 border-blue-500 shadow-sm flex items-start space-x-4 opacity-70">
                                    <div className="bg-blue-50 text-blue-500 p-3 rounded-lg font-bold text-sm text-center min-w-[60px] flex flex-col justify-center"><div>STOP</div><div className="text-lg">2</div></div>
                                    <div className="flex-1 py-1"><div className="flex justify-between items-center mb-1"><span className="font-bold text-lg text-slate-800">7-Eleven (海淀黄庄)</span></div><div className="text-sm text-slate-500">任务: 新品陈列检查</div></div>
                                </div>
                            </div>
                            
                            <motion.button whileTap={{ scale: 0.98 }} className="w-full bg-[#07c160] hover:bg-[#06ad56] text-white py-4 rounded-xl font-bold text-lg shadow-lg flex items-center justify-center mt-4 transition-colors">
