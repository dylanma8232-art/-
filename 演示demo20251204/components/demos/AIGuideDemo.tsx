...
import { Mic, Radio, Activity, ChevronRight, Star, AlertCircle, FileCheck, PlayCircle, Loader2 } from 'lucide-react';
...
const AIGuideDemo: React.FC<{ onComplete?: () => void; timeline?: any }> = ({ onComplete, timeline: propTimeline }) => {
  ...
  const [transcript, setTranscript] = useState<{sender: 'ai' | 'user' | 'system', text: string}[]>([]);
  const simulationRef = useRef<boolean>(false);
  const chatContainerRef = useRef<HTMLDivElement>(null);

  // Robustly merge timeline
  const defaultTimeline = { workbenchStay: 2000, intro: 2000, userSpeech: 12000, thinking: 25000, feedback: 30000, report: 42000, complete: 52000 };
  const timeline = { ...defaultTimeline, ...propTimeline };

  useEffect(() => {
    let autoStartTimer: ReturnType<typeof setTimeout>;
    // Auto-start logic if not started manually
  ...
  const startSimulation = () => {
    ...
    // Add initial system message to prevent empty black screen feeling
    setTranscript([{ sender: 'system', text: '正在连接 AI 陪练系统...' }]);

    // Schedule AI speech
    setTimeout(() => { 
        setAvatarState('speaking'); 
        setTranscript(prev => [...prev.filter(m => m.sender !== 'system'), { sender: 'ai', text: '你好，我是您的 AI 陪练。今天的课题是《新品眼霜的连带销售》。请尝试向我推荐这款产品。' }]); 
    }, timeline.intro); 
    
    setTimeout(() => { 
        setAvatarState('listening'); 
        setTranscript(prev => [...prev, { sender: 'user', text: '您好，这款眼霜特别添加了高浓度玻色因，能有效淡化细纹。而且现在我们在做活动，搭配这款按摩仪一起买，效果更好，价格也非常划算。' }]); 
    }, timeline.userSpeech);
    ...
    setTimeout(() => { 
        setAvatarState('speaking'); 
        setTranscript(prev => [...prev, { sender: 'ai', text: '语速适中，连带销售切入得很自然。不过在介绍玻色因成分时，可以更具体一点，比如“像给肌肤喝饱水”。综合评分生成中...' }]); 
    }, timeline.feedback);
  ...
  return (
    <div className="h-full flex flex-col relative font-sans bg-slate-900 items-center justify-center">
      ...
      <AnimatePresence>
      {mode === 'intro' && (
         <motion.div initial={{ opacity: 1 }} exit={{ opacity: 0, y: -50 }} className="absolute inset-0 z-50 flex items-center justify-center bg-black/60">
            <TabletWrapper>
      ...
      <div className="flex-1 p-0 h-full w-full max-w-[1600px] overflow-hidden relative flex flex-col items-center justify-center">
         {mode === 'intro' ? (<div className="flex items-center justify-center h-full text-slate-500">Loading Simulation...</div>) : mode === 'report' ? (
            ...
            <div className="h-full w-full flex flex-col gap-4 p-4 lg:p-8">
                ...
                <div className="flex-1 bg-slate-800/50 rounded-3xl border border-white/5 overflow-hidden relative">
                    <div ref={chatContainerRef} className="absolute inset-0 p-6 lg:p-8 overflow-y-auto space-y-6 custom-scrollbar scroll-smooth">
                       {transcript.map((t, i) => (
                          <motion.div key={i} initial={{ opacity: 0, x: t.sender === 'ai' ? -20 : 20 }} animate={{ opacity: 1, x: 0 }} className={`flex ${t.sender === 'ai' ? 'justify-start' : t.sender === 'system' ? 'justify-center' : 'justify-end'}`}>
                             {t.sender === 'system' ? (
