...
// Replaced import with local definition to fix resolution error
...
const BRAND_LOGOS = [
  { image: getPublicPath("/dll.png") },
  { image: getPublicPath("/hly.png") },
  { image: getPublicPath("/jiale.png") },
  { image: getPublicPath("/pepsico.png") },
  { image: getPublicPath("/ssd.png") },
  { image: getPublicPath("/vinda.png") },
  { image: getPublicPath("/yhjl.png") },
  { image: getPublicPath("/ykd.png") },
  { image: getPublicPath("/zespri.png") },
];
...
const CompanyIntro: React.FC<{ active?: boolean; onComplete?: () => void }> = ({ active = false }) => {
  return (
    <div className={`h-full w-full relative overflow-hidden flex flex-col font-sans text-white transition-all duration-1000 
      ...
      {/* Content Container - Expanded max-width for ultra-wide screens */}
      {/* 2xl:max-w-[90%] ensures 3072px width is utilized nicely without being too stretched */}
      <div className="relative z-10 flex-1 flex flex-col justify-between items-center w-full h-full max-w-[95%] 2xl:max-w-[90%] mx-auto p-4 md:p-8 lg:p-10 pb-20 lg:pb-24">
        ...
        >
          <div className="mt-[-140px] flex items-center justify-center gap-6 mb-2">
             <img src={getPublicPath("/logo.png")} alt="RetailAIM Logo" className="h-[400px]" />
          </div>
          <h2 className="mt-[-140px] text-4xl lg:text-5xl font-bold text-white tracking-wide drop-shadow-[0_0_20px_rgba(255,255,255,0.3)]">全域零售 AI 解决方案专家</h2>
          <h3 className="text-lg lg:text-xl font-light text-slate-300 tracking-[0.15em] uppercase opacity-90 mt-2">
            打通即时零售与线下门店协同增长的全渠道引擎
          </h3>
        </motion.div>

        {/* 2. CORE METRICS (Adjusted heights and gap for wide screens) */}
        <div className="w-full grid grid-cols-5 gap-4 lg:gap-12 2xl:gap-16 px-4 items-end">
        ...
        {/* 3. MODULES (Reduced height, wider max-width) */}
        <div className="w-full max-w-[1600px] 2xl:max-w-[2400px] flex flex-col items-center relative z-20">
           <div className="grid grid-cols-3 gap-6 lg:gap-10 2xl:gap-16 w-full">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.5, duration: 1 }} className="w-full max-w-[1400px] 2xl:max-w-[1800px]">
                    ...
                    <div className="flex animate-marquee items-center gap-8 h-16">
                      {[...BRAND_LOGOS, ...BRAND_LOGOS].map((brand, i) => (
                          <div key={i} className="flex flex-col items-center gap-1 shrink-0 opacity-70 hover:opacity-100 transition-all cursor-default group">
                             <div className="w-24 h-10 bg-white border border-white/20 rounded flex items-center justify-center relative overflow-hidden group-hover:shadow-lg transition-all p-2">
                                <img src={brand.image} alt="Brand logo" className="h-full w-full object-contain" />
                             </div>
                          </div>
                      ))}
                    </div>
