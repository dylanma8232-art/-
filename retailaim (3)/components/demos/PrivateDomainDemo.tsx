...
const TabletWrapper: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <div className="transform scale-90 md:scale-100 lg:scale-110 2xl:scale-125 transition-transform duration-500 origin-center">
    <div className="w-[680px] h-[900px] bg-[#ededed] shadow-2xl flex flex-col overflow-hidden border-[12px] border-slate-800 rounded-[45px]">
        {children}
    </div>
  </div>
);
...
const PrivateDomainDemo: React.FC<{ onComplete?: () => void }> = ({ onComplete }) => {
  ...
  useEffect(() => {
    ...
    const script = [
      ...
      { t: delay + 3000, action: () => setShowAISuggestion(true) }, 
      { t: delay + 5500, action: () => { setShowAISuggestion(false); setIsTyping(true); } },
      { t: delay + 7500, action: () => { setIsTyping(false); addMsg({ id: 2, type: 'agent', content: '宝妈您好！宝宝多大了呢？最近转季很多宝宝会有厌奶期。我们刚到了款“小金领”适合肠胃娇嫩的宝宝，口味很清淡。', timestamp: 0 }); } },
      { t: delay + 9500, action: () => addMsg({ id: 3, type: 'agent', content: '给您发个附近门店的小程序，现在下单有活动，30分钟就能送到家试用一下。', card: { title: '美团超市 - 世纪公园店', desc: '距离您 2.8km | 预计 30分钟送达', image: 'Store' }, timestamp: 0 }) },
      { t: delay + 12500, action: () => setShowCommission(true) }, 
      { t: delay + 15500, action: () => addMsg({ id: 4, type: 'user', content: '已经下单了，显示马上就送过来，真快！谢谢啦', timestamp: 0 }) },
      { t: delay + 18000, action: () => onComplete && onComplete() }
    ];
  ...
  return (
    <div className="h-full w-full flex items-center justify-center bg-slate-900 relative overflow-hidden rounded-2xl font-sans text-slate-900 border border-white/10">
      <TabletWrapper>
        ...
        <div className="flex-1 p-8 overflow-y-auto space-y-8 relative scroll-smooth">
