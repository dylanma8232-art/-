...
export interface IntroConfig {
  ...
  role: string;
  goal: string;
  duration?: number; // Defaults to 3500ms if not specified
}
...
export interface SceneConfig {
  ...
  duration: number; // Duration of the CONTENT part (ms)
  intro?: IntroConfig; // If present, plays this intro before mounting component
  steps?: SceneStep[];
}
...
export const DEMO_PLAYLIST: SceneConfig[] = [
  { 
    ...
    duration: 15000,
    // No intro for the landing page itself
    steps: [
      { name: 'Logo入场', delay: 0 },
